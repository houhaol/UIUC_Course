...
std::vector<Edge> NimLearner::playRandomGame() const {
  vector<Edge> path;
 /* Your code goes here! */
 int length = g_.getVertices().size() /2;
 Vertex tmp ="p1-" +to_string(length - 1);

 while (tmp != ("p1-" +to_string(0)) &&tmp != ("p2-" +to_string(0)) )
 {
   vector<Vertex> neighbor = g_.getAdjacent(tmp);
   int r = rand() % neighbor.size();
   path.push_back(g_.getEdge(tmp, neighbor[r]));
   tmp = neighbor[r]; 
 }
...
void NimLearner::updateEdgeWeights(const std::vector<Edge> & path) {
 ...
 {
   Edge e = path[idx];
   g_.setEdgeWeight(e.source, e.dest, bonus + (g_.getEdgeWeight(e.source, e.dest)));
   bonus *= -1;
   idx --;
 }
