...
void Allocator::createLetterGroups()
{
    ...
    alpha = new Letter[26];
    for (int i = 0; i < 26; i++)
        alpha[i].letter = 'A' + i; // iterate from A to B, now alpha[26] contains alpha[0] = 'A', alpha[1] = 'B', ..., alpha[25] = 'Z'
    std::cout << &alpha[1].letter << std::endl;
}
...
void Allocator::loadRooms(const std::string& file)
{
    ...
    while (fileio::areMoreRooms()) {
        rooms[i] = fileio::nextRoom(); // here = call operator=, where it has copy ctor
