...
class KDTree
{
  ...
  private:
    ...
    // helper function for build tree
    KDTreeNode* buildTree(int dim, int left, int right);
    
    // helper function for selecting the pivotIndex
    void select(int left, int right, int middle, int dim);

    // helper function for sorting the data
    ...
    // Point<Dim> nearestNeighbor_(const Point<Dim>& query, int left, int right, int dim) const;
    Point<Dim> nearestNeighbor_(const Point<Dim>& query, typename KDTree<Dim>::KDTreeNode* curNode, int curDim) const;
