...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(word1_.begin(), word1_.end(), word1_.begin(), ::toupper);
    std::transform(word2_.begin(), word2_.end(), word2_.begin(), ::toupper);
    auto it1 = dict.find(word1_);
    auto it2 = dict.find(word2_);
    ...
    vector<std::string> pronounce1 = dict.find(word1_)->second;
    vector<std::string> pronounce2 = dict.find(word2_)->second;

    // comparsion
    if (pronounce1.size()==0 || pronounce2.size() == 0 || pronounce1.size() != pronounce2.size())
    {
        return false;
    }
    
    for (unsigned int i = 0; i < pronounce1.size(); i++)
    {
        if (pronounce1[i] != pronounce2[i])
        {
            return false;
        }
