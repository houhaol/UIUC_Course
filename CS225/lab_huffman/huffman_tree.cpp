                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    TreeNode * singleTop = singleQueue.front();
    TreeNode * mergeTop = mergeQueue.front();

    if (singleTop->freq.getFrequency() <= mergeTop -> freq.getFrequency())
    {
        singleQueue.pop();
        return singleTop;
    }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    // First part, enqueue all node into singleQueue
    ...
    {
        TreeNode * Node1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode * Node2 = removeSmallest(singleQueue, mergeQueue);
        /* if (Node1 == NULL)
        {
            return;
        }
        if (Node2 == NULL)
        {
            TreeNode * newNode = new TreeNode(Node1->freq.getFrequency());
            mergeQueue.push(newNode);
            return;
        } */
        
        TreeNode * newNode = new TreeNode(Node1->freq.getFrequency() + Node2->freq.getFrequency());
        // TreeNode * newNode = NULL;
        // newNode->freq = (Node1->freq.getFrequency() + Node2->freq.getFrequency());
        newNode->left = Node1;
        newNode->right = Node2;
        mergeQueue.push(newNode);
    }
